...
var MaterialUiBigDialog = React.createClass({
	...
	render: function() {
	  if(this.state.displayDialog === true) {
		var textArea = React.createElement('textarea', {style: textareaStyle}, this.props.text);
		var subtitle = React.createElement('h2', {style: { fontFamily: 'roboto', marginLeft: 5 + 'px' }, dangerouslySetInnerHTML:{__html:this.props.rating}});
		...
		if(this.state.isOwner === true) {
				...
				return React.createElement('div', {style: materialDialogBoxStyle}, closeBoxCross, title, subtitle, textArea, submitButton, cancelButton);
		} else {
				var closeButton = React.createElement(FlatButton, {onClick: this.closeDialog, style: {float: 'right'}, label: "Close", default: true});
				return React.createElement('div', {style: materialDialogBoxStyle}, closeBoxCross, title, subtitle, textArea, closeButton);
		}
	  }
	  return null;
  }
